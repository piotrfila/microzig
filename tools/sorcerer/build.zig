...
fn generate_zig_schema_literal(arena: std.mem.Allocator, schemas: []const RegisterSchemaUsage) ![]const u8 {
    ...
    const NormalizedPath = struct {
        ...
        pub fn format(
            ...
            var tail = self.s;
            while (std.mem.indexOfScalar(u8, tail, '\\')) |pos| {
                try w.writeAll(tail[0..pos]);
                try w.writeByte('/');
                tail = tail[pos + 1 ..];
            }
            try w.writeAll(tail);
                ...
                for (chip.patch_files) |patch_file| {
                    switch (patch_file) {
                        .src_path => |src| {
                            ...
                            try w.print("                        .sub_path = \"{f}\",\n", .{NormalizedPath{ .s = src.sub_path }});
                            try w.print("                        .build_root = \"{f}\",\n", .{NormalizedPath{ .s = src.build_root }});
                            try w.writeAll("                    } },\n");
                        },
                        .dependency => |dep| {
                            try w.writeAll("                    .{ .dependency = .{\n");
                            try w.print("                        .sub_path = \"{f}\",\n", .{NormalizedPath{ .s = dep.sub_path }});
                            try w.print("                        .build_root = \"{f}\",\n", .{NormalizedPath{ .s = dep.build_root }});
                            try w.print("                        .dep_name = \"{s}\",\n", .{dep.dep_name});
                            try w.writeAll("                    } },\n");
                        },
                    }
                }
